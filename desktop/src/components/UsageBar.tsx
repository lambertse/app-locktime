      ...
      <div className={cn('flex items-center gap-2', className)}>
        <div className="flex-1 h-2 bg-zinc-800 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min((minutesUsed / 120) * 100, 100)}%` }}
          />
        </div>
        {showLabel && (
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      ...
      {showLabel && (
        <span
          className={cn(
            'text-xs font-mono w-24 shrink-0 text-right',
            exceeded ? 'text-red-400' : 'text-zinc-400',
          )}
        >
          {exceeded
            ? 'LIMIT REACHED'
            : `${formatMinutes(minutesUsed)} / ${formatMinutes(dailyLimitMinutes)}`}
        </span>
      )}
    </div>
  )
