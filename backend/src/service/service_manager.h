...
class ServiceManager {
 public:
  ...
  static std::error_code install_service(const std::string& exe_path);

  /// Remove the service from the OS service manager.
  static std::error_code uninstall_service();

  /// Run the service event loop.
  /// On Windows: calls StartServiceCtrlDispatcher, returns when service stops.
  /// On macOS: runs an event loop with signal handling (launchd keeps it
  /// alive).
