...
static std::error_code serial_err() {
    return ibridger::common::make_error_code(ibridger::common::Error::serialization_error);
}
...
static void fill_proto_schedule(const locktime::Schedule& s, ::locktime::rpc::Schedule* p) {
    p->set_id(s.id);
...
void LockTimeService::fill_proto_rule(const locktime::Rule& r, ::locktime::rpc::Rule* p) {
    p->set_id(r.id);
    p->set_name(r.name);
...
static locktime::Schedule schedule_from_payload(const ::locktime::rpc::SchedulePayload& p,
...
LockTimeService::LockTimeService(std::shared_ptr<Database> db,
    ...
    , started_at_(started_at)
{
    ...
    register_method("CreateRule",       [this](auto& p){ return handle_create_rule(p); });
    register_method("UpdateRule",       [this](auto& p){ return handle_update_rule(p); });
    register_method("PatchRule",        [this](auto& p){ return handle_patch_rule(p); });
    register_method("DeleteRule",       [this](auto& p){ return handle_delete_rule(p); });
    register_method("GrantOverride",    [this](auto& p){ return handle_grant_override(p); });
    register_method("RevokeOverride",   [this](auto& p){ return handle_revoke_override(p); });
    register_method("GetUsageToday",    [this](auto& p){ return handle_get_usage_today(p); });
    register_method("GetUsageWeek",     [this](auto& p){ return handle_get_usage_week(p); });
    register_method("GetBlockAttempts", [this](auto& p){ return handle_get_block_attempts(p); });
    register_method("GetProcesses",     [this](auto& p){ return handle_get_processes(p); });
    register_method("GetConfig",        [this](auto& p){ return handle_get_config(p); });
    register_method("UpdateConfig",     [this](auto& p){ return handle_update_config(p); });
...
LockTimeService::handle_get_status(const std::string& payload) {
    ...
    svc->set_uptime_seconds(
        static_cast<int64_t>(std::chrono::duration_cast<std::chrono::seconds>(elapsed).count()));
    svc->set_time_synced(true);
    svc->set_ntp_offset_ms(0);

    // Rule statuses
    for (const auto& rule : rules) {
        int minutes_used = db_->get_daily_minutes(rule.id, utils::today_date(), now);
...
std::pair<std::string, std::error_code>
LockTimeService::handle_list_rules(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_get_rule(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_create_rule(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_update_rule(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_patch_rule(const std::string& payload) {
    ...
    db_->patch_rule(req.id(),
                    req.has_enabled(), req.enabled(),
                    req.has_name(),    req.name());
...
std::pair<std::string, std::error_code>
LockTimeService::handle_delete_rule(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_grant_override(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_revoke_override(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_get_usage_today(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_get_usage_week(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_get_processes(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_get_config(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_update_config(const std::string& payload) {
...
std::pair<std::string, std::error_code>
LockTimeService::handle_check_app(const std::string& payload) {
    ...
    for (char& c : exe_lower) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
    ...
    for (const auto& rule : rules) {
        ...
        if (rule.match_mode == "path") {
            ...
            for (char& c : rule_lower) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
...
void LockTimeService::set_ifeo(const std::string& exe_name, const std::string& blocker_path) {
    std::wstring key_path = L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\"
    ...
    HKEY hKey = nullptr;
    DWORD disposition = 0;
    LONG rc = RegCreateKeyExW(HKEY_LOCAL_MACHINE, key_path.c_str(),
                               0, nullptr, REG_OPTION_NON_VOLATILE,
                               KEY_SET_VALUE, nullptr, &hKey, &disposition);
