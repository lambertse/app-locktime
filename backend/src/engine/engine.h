...
/// Returns true if the time-of-day component of `now` (UTC) falls in [allow_start, allow_end).
/// Handles overnight windows (allow_end < allow_start) correctly.
...
/// Returns the next time (after `now`) the rule will transition from locked→active.
/// Returns nullopt if no schedules exist (permanently blocked).
std::optional<std::time_t> next_unlock_at(const std::vector<Schedule>& schedules, std::time_t now);

/// Returns the next time (after `now`) the rule will transition from active→locked.
/// Returns nullopt if currently not in any schedule.
