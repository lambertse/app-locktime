...
// Returns the next time_t (strictly after `now`) when the given DOW and HH:MM occur.
// Uses UTC throughout.
std::time_t next_occurrence_of(int target_dow, const std::string& hhmm, std::time_t now) {
    int h = 0, m = 0;
...
bool is_in_schedule(const Schedule& sched, std::time_t now) {
    ...
    return contains_day(sched.days, t.tm_wday) && now_m >= start_m && now_m < end_m;
}
...
RuleStatus compute_rule_status(const Rule& rule, int minutes_used_today, std::time_t now) {
    ...
    bool limit_ok  = (rule.daily_limit_minutes == 0) || (minutes_used_today < rule.daily_limit_minutes);
    ...
    if (!in_window) {
        ...
        // Limit is the only reason: next unlock is midnight (start of new day, UTC).
        struct tm t = utc_tm_of(now);
        t.tm_hour  = 0;
        t.tm_min   = 0;
        t.tm_sec   = 0;
