...
Watcher::Watcher(std::shared_ptr<Database> db)
    : db_(std::move(db))
{}
...
std::vector<ProcessEntry> Watcher::enumerate_processes() {
    ...
    do {
        ...
        if (len > 0) {
            ...
            WideCharToMultiByte(CP_UTF8, 0, pe.szExeFile, -1, &narrow[0], len, nullptr, nullptr);
        ...
        if (hProc) {
            ...
            if (QueryFullProcessImageNameW(hProc, 0, path, &size)) {
                int plen = WideCharToMultiByte(CP_UTF8, 0, path, -1, nullptr, 0, nullptr, nullptr);
                if (plen > 0) {
                    std::string pnarrow(plen, '\0');
                    WideCharToMultiByte(CP_UTF8, 0, path, -1, &pnarrow[0], plen, nullptr, nullptr);
...
void Watcher::reconcile(const std::vector<ProcessEntry>& procs, std::time_t now) {
    ...
    for (const auto& rule : rules) {
        ...
        for (char& c : rule_low) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
        ...
        if (status.status == "active") {
            ...
            if (is_running && active_sessions_.find(rule.id) == active_sessions_.end()) {
            ...
            if (is_running) {
                ...
                db_->insert_audit("watcher_kill", rule.id,
