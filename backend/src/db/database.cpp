...
void Database::seed_config() {
    exec_stmt(
        ...
        "('ntp_enabled','true'),('ntp_offset_tolerance_ms','300000'),('timezone','UTC');");
}
...
std::vector<Schedule> Database::load_schedules_for_rule_locked(const std::string& rule_id) {
...
std::optional<Rule> Database::get_rule_by_id(const std::string& id) {
    ...
    const char* sql =
        "SELECT id, name, exe_name, exe_path, match_mode, enabled, "
        "daily_limit_minutes, ifeo_active, created_at, updated_at FROM rules WHERE id = ?;";
    sqlite3_stmt* stmt = nullptr;
    ...
    for (const auto& sched : r.schedules) {
        exec_stmt(
            ...
            [&](sqlite3_stmt* s) {
                ...
                sqlite3_bind_text(s, 4, sched.allow_start.c_str(), -1, SQLITE_TRANSIENT);
                sqlite3_bind_text(s, 5, sched.allow_end.c_str(),   -1, SQLITE_TRANSIENT);
...
void Database::update_rule(const Rule& r) {
    ...
    // Replace schedules
    exec_stmt("DELETE FROM schedules WHERE rule_id=?;",
        [&](sqlite3_stmt* s) {
            sqlite3_bind_text(s, 1, r.id.c_str(), -1, SQLITE_TRANSIENT);
        });
    for (const auto& sched : r.schedules) {
...
void Database::patch_rule(const std::string& id,
...
void Database::delete_rule(const std::string& id) {
    std::lock_guard<std::mutex> lock(mu_);
    exec_stmt("DELETE FROM rules WHERE id=?;",
        [&](sqlite3_stmt* s) {
...
void Database::set_rule_ifeo_active(const std::string& id, bool active) {
    std::lock_guard<std::mutex> lock(mu_);
    exec_stmt("UPDATE rules SET ifeo_active=? WHERE id=?;",
        [&](sqlite3_stmt* s) {
...
std::vector<Schedule> Database::get_schedules_for_rule(const std::string& rule_id) {
    std::lock_guard<std::mutex> lock(mu_);
...
int64_t Database::open_session(const std::string& rule_id, int pid, std::time_t started_at) {
    std::lock_guard<std::mutex> lock(mu_);
    auto ts    = utils::format_iso8601(started_at);
    auto date  = ts.substr(0, 10);  // "YYYY-MM-DD"

    exec_stmt(
        "INSERT INTO usage_sessions(rule_id, date, pid, started_at) VALUES(?,?,?,?);",
        [&](sqlite3_stmt* s) {
            sqlite3_bind_text(s, 1, rule_id.c_str(), -1, SQLITE_TRANSIENT);
...
void Database::close_session(int64_t session_id, std::time_t ended_at,
    ...
    if (sqlite3_prepare_v2(db_, "SELECT started_at FROM usage_sessions WHERE id=?;",
    ...
    exec_stmt(
        "UPDATE usage_sessions SET ended_at=?, duration_minutes=?, terminated_by=? WHERE id=?;",
        [&](sqlite3_stmt* s) {
...
void Database::crash_recovery(std::time_t startup_time) {
    ...
    exec_stmt(
        ...
        "duration_minutes = CAST((strftime('%s',?) - strftime('%s',started_at)) / 60 AS INTEGER) "
...
std::vector<UsageSession> Database::get_sessions_for_date(const std::string& date) {
    std::lock_guard<std::mutex> lock(mu_);
    std::vector<UsageSession> result;
    const char* sql =
...
void Database::create_override(const Override& o) {
    std::lock_guard<std::mutex> lock(mu_);
    exec_stmt(
        "INSERT INTO overrides(rule_id, granted_at, expires_at, duration_minutes, reason, consumed) "
        "VALUES(?,?,?,?,?,?);",
        [&](sqlite3_stmt* s) {
...
std::optional<Override> Database::get_active_override(const std::string& rule_id,
                                                        std::time_t now) {
    std::lock_guard<std::mutex> lock(mu_);
...
bool Database::delete_active_override(const std::string& rule_id, std::time_t now) {
    std::lock_guard<std::mutex> lock(mu_);
    auto now_str = utils::format_iso8601(now);
    ...
    {
        const char* sql =
            "SELECT id FROM overrides WHERE rule_id=? AND expires_at > ? AND consumed=0 LIMIT 1;";
        sqlite3_stmt* stmt = nullptr;
        if (sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr) == SQLITE_OK) {
            sqlite3_bind_text(stmt, 1, rule_id.c_str(), -1, SQLITE_TRANSIENT);
            sqlite3_bind_text(stmt, 2, now_str.c_str(), -1, SQLITE_TRANSIENT);
